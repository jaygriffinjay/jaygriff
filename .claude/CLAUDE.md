Claude Instructions for `jaygriff`
...
This is a modern web application built with **Next.js** and **TypeScript**. The project uses **Emotion** for styling. Key directories include:
...
- **Primitive Components**: design system with primitives exported from `src/components/Primitives.tsx`. Use these as building blocks instead of creating one-off components.
...
- **Rich Metadata for Content Pages**: When generating content files in `content/tsx/`, you are typically the second author. Add thoughtful, rich metadata to the `PostMeta` export:
  ...
  - Ensure `slug` is kebab-case and immediately descriptive and obvious as to what the content contains. Think "clickbait title in the form of slug".
